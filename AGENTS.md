...
17. **Do not wire Worker Builds to the frontend** — the `lawgpt-edge` Worker Git integration must not use `/frontend` or Pages commands. GitHub Actions owns production deploys; if Cloudflare Workers Builds are enabled, point them at `infra` or disable non-production branch builds. The default preview command `wrangler versions upload` fails from `/frontend` with "Missing entry-point".
